    ...
    extends JpaRepository<DemandePriseEnCharge, Long>, JpaSpecificationExecutor<DemandePriseEnCharge>
{
    ...
    @Query(
        value = "select demandePriseEnCharge from DemandePriseEnCharge demandePriseEnCharge left join fetch demandePriseEnCharge.gestionnaireCreateur left join fetch demandePriseEnCharge.assigneA",
    ...
    @Query(
        "select demandePriseEnCharge from DemandePriseEnCharge demandePriseEnCharge left join fetch demandePriseEnCharge.gestionnaireCreateur left join fetch demandePriseEnCharge.assigneA"
    )
    List<DemandePriseEnCharge> findAllWithToOneRelationships();

    @Query(
        "select demandePriseEnCharge from DemandePriseEnCharge demandePriseEnCharge left join fetch demandePriseEnCharge.gestionnaireCreateur left join fetch demandePriseEnCharge.assigneA where demandePriseEnCharge.id =:id"
    )
