...
import { TacheDeleteDialog } from '../delete/tache-delete-dialog';
...
export class Tache {
  ...
  readonly isLoading = this.tacheService.tachesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.tacheService.tachesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.taches.set(this.fillComponentAttributesFromResponseBody([...this.tacheService.taches()]));
    });
    effect(() => {
  ...
  trackId = (item: ITache): number => this.tacheService.getTacheIdentifier(item);

  delete(tache: ITache): void {
    const modalRef = this.modalService.open(TacheDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: ITache[]): ITache[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.tacheService.tachesParams.set(queryObject);
  }
