...
import carteBeneficiaireResolve from './carte-beneficiaire-routing-resolve.service';

describe('CarteBeneficiaire routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICarteBeneficiaire returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          carteBeneficiaireResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(carteBeneficiaireResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
