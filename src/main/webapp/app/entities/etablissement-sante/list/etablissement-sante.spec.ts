...
import { sampleWithRequiredData } from '../etablissement-sante.test-samples';
import { EtablissementSanteService } from '../service/etablissement-sante.service';

import { EtablissementSante } from './etablissement-sante';

vi.useFakeTimers();

describe('EtablissementSante Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: EtablissementSante;
  let fixture: ComponentFixture<EtablissementSante>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EtablissementSante);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 2850 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.etablissementSantes()[0]).toEqual(expect.objectContaining({ id: 2850 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 2850 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.etablissementSantes()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 2850 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to etablissementSanteService', () => {
      ...
      vi.spyOn(service, 'getEtablissementSanteIdentifier');
      const id = comp.trackId(entity);
      expect(service.getEtablissementSanteIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.etablissementSantesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.etablissementSantesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });
