...
import { DirectionDeleteDialog } from '../delete/direction-delete-dialog';
...
export class Direction {
  readonly directions = signal<IDirection[]>([]);
  ...
  readonly isLoading = this.directionService.directionsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.directionService.directionsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.directions.set(this.fillComponentAttributesFromResponseBody([...this.directionService.directions()]));
    });
    effect(() => {
  ...
  trackId = (item: IDirection): number => this.directionService.getDirectionIdentifier(item);

  delete(direction: IDirection): void {
    const modalRef = this.modalService.open(DirectionDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected queryBackend(): void {
    ...
    this.directionService.directionsParams.set(queryObject);
  }
