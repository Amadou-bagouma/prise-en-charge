...
import { sampleWithRequiredData } from '../gestion.test-samples';
...
describe('Gestion Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 26677 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.gestions()[0]).toEqual(expect.objectContaining({ id: 26677 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 26677 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.gestions()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 26677 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to gestionService', () => {
      ...
      vi.spyOn(service, 'getGestionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getGestionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.gestionsParams()).toMatchObject({ sort: ['id,desc'] });
  });
