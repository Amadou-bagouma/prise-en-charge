...
import { sampleWithRequiredData } from '../ayant-droit.test-samples';
...
describe('AyantDroit Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AyantDroit;
  let fixture: ComponentFixture<AyantDroit>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AyantDroit);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 17970 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.ayantDroits()[0]).toEqual(expect.objectContaining({ id: 17970 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 17970 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.ayantDroits()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 17970 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to ayantDroitService', () => {
      ...
      vi.spyOn(service, 'getAyantDroitIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAyantDroitIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.ayantDroitsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.ayantDroitsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });
