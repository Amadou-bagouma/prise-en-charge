...
import { BoiteReceptionDeleteDialog } from '../delete/boite-reception-delete-dialog';
...
export class BoiteReception {
  readonly boiteReceptions = signal<IBoiteReception[]>([]);
  ...
  readonly isLoading = this.boiteReceptionService.boiteReceptionsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.boiteReceptionService.boiteReceptionsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.boiteReceptions.set(this.fillComponentAttributesFromResponseBody([...this.boiteReceptionService.boiteReceptions()]));
    });
    effect(() => {
  ...
  trackId = (item: IBoiteReception): number => this.boiteReceptionService.getBoiteReceptionIdentifier(item);

  delete(boiteReception: IBoiteReception): void {
    const modalRef = this.modalService.open(BoiteReceptionDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.boiteReception = boiteReception;
  ...
  protected fillComponentAttributesFromResponseBody(data: IBoiteReception[]): IBoiteReception[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.boiteReceptionService.boiteReceptionsParams.set(queryObject);
  }
