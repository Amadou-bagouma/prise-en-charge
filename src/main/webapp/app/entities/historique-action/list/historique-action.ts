...
import { HistoriqueActionDeleteDialog } from '../delete/historique-action-delete-dialog';
...
@Component({
  selector: 'jhi-historique-action',
  templateUrl: './historique-action.html',
  imports: [
    RouterLink,
    FontAwesomeModule,
    AlertError,
...
export class HistoriqueAction {
  readonly historiqueActions = signal<IHistoriqueAction[]>([]);
  ...
  readonly isLoading = this.historiqueActionService.historiqueActionsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.historiqueActionService.historiqueActionsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.historiqueActions.set(this.fillComponentAttributesFromResponseBody([...this.historiqueActionService.historiqueActions()]));
    });
    effect(() => {
  ...
  trackId = (item: IHistoriqueAction): number => this.historiqueActionService.getHistoriqueActionIdentifier(item);

  delete(historiqueAction: IHistoriqueAction): void {
    const modalRef = this.modalService.open(HistoriqueActionDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.historiqueAction = historiqueAction;
  ...
  protected fillComponentAttributesFromResponseBody(data: IHistoriqueAction[]): IHistoriqueAction[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.historiqueActionService.historiqueActionsParams.set(queryObject);
  }
