...
import { PieceJustificativeDeleteDialog } from '../delete/piece-justificative-delete-dialog';
...
export class PieceJustificative {
  readonly pieceJustificatives = signal<IPieceJustificative[]>([]);
  ...
  readonly isLoading = this.pieceJustificativeService.pieceJustificativesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.pieceJustificativeService.pieceJustificativesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.pieceJustificatives.set(this.fillComponentAttributesFromResponseBody([...this.pieceJustificativeService.pieceJustificatives()]));
    });
    effect(() => {
  ...
  trackId = (item: IPieceJustificative): number => this.pieceJustificativeService.getPieceJustificativeIdentifier(item);

  delete(pieceJustificative: IPieceJustificative): void {
    const modalRef = this.modalService.open(PieceJustificativeDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.pieceJustificative = pieceJustificative;
  ...
  protected fillComponentAttributesFromResponseBody(data: IPieceJustificative[]): IPieceJustificative[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.pieceJustificativeService.pieceJustificativesParams.set(queryObject);
  }
