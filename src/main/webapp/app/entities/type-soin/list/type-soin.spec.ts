...
import { sampleWithRequiredData } from '../type-soin.test-samples';

import { TypeSoin } from './type-soin';

vi.useFakeTimers();

describe('TypeSoin Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TypeSoin;
  let fixture: ComponentFixture<TypeSoin>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TypeSoin);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 20111 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.typeSoins()[0]).toEqual(expect.objectContaining({ id: 20111 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 20111 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.typeSoins()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 20111 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to typeSoinService', () => {
      ...
      vi.spyOn(service, 'getTypeSoinIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTypeSoinIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.typeSoinsParams()).toMatchObject({ sort: ['id,desc'] });
  });
