...
import demandePriseEnChargeResolve from './demande-prise-en-charge-routing-resolve.service';

describe('DemandePriseEnCharge routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IDemandePriseEnCharge returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          demandePriseEnChargeResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(demandePriseEnChargeResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
