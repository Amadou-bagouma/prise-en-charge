...
import { sampleWithRequiredData } from '../piece-justificative.test-samples';
...
describe('PieceJustificative Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PieceJustificative;
  let fixture: ComponentFixture<PieceJustificative>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PieceJustificative);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 17302 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.pieceJustificatives()[0]).toEqual(expect.objectContaining({ id: 17302 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 17302 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.pieceJustificatives()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 17302 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to pieceJustificativeService', () => {
      const entity = { id: 17302 };
      vi.spyOn(service, 'getPieceJustificativeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPieceJustificativeIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.pieceJustificativesParams()).toMatchObject({ sort: ['id,desc'] });
  });
