...
import etablissementSanteResolve from './etablissement-sante-routing-resolve.service';

describe('EtablissementSante routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IEtablissementSante returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          etablissementSanteResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(etablissementSanteResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
