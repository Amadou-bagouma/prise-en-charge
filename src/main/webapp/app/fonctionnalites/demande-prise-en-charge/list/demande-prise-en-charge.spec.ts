...
import { sampleWithRequiredData } from '../demande-prise-en-charge.test-samples';
import { DemandePriseEnChargeService } from '../service/demande-prise-en-charge.service';

import { DemandePriseEnCharge } from './demande-prise-en-charge';

vi.useFakeTimers();

describe('DemandePriseEnCharge Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: DemandePriseEnCharge;
  let fixture: ComponentFixture<DemandePriseEnCharge>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(DemandePriseEnCharge);
  ...
  /**
   * L'écran charge la liste et, en même temps, les effectifs des quatre onglets. Les specs
   * ne s'intéressent qu'à la liste : ce raccourci écarte les appels de comptage, qui partent
   * vers `/count`.
   */
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 17525 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.demandePriseEnCharges()[0]).toEqual(expect.objectContaining({ id: 17525 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 17525 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.demandePriseEnCharges()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = requeteListe();
    successReq.flush([{ id: 17525 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to demandePriseEnChargeService', () => {
      ...
      vi.spyOn(service, 'getDemandePriseEnChargeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDemandePriseEnChargeIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.demandePriseEnChargesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.demandePriseEnChargesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });
