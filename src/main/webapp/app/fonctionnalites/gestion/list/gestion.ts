...
import { GestionDeleteDialog } from '../delete/gestion-delete-dialog';
...
export class Gestion {
  ...
  protected readonly gestionService = inject(GestionService);
  // eslint-disable-next-line @typescript-eslint/member-ordering
  readonly isLoading = this.gestionService.gestionsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.gestionService.gestionsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.gestions.set(this.fillComponentAttributesFromResponseBody([...this.gestionService.gestions()]));
    });
    effect(() => {
  ...
  trackId = (item: IGestion): number => this.gestionService.getGestionIdentifier(item);

  delete(gestion: IGestion): void {
    const modalRef = this.modalService.open(GestionDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IGestion[]): IGestion[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.gestionService.gestionsParams.set(queryObject);
  }
