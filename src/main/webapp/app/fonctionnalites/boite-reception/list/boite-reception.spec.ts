...
import { sampleWithRequiredData } from '../boite-reception.test-samples';
import { BoiteReceptionService } from '../service/boite-reception.service';

import { BoiteReception } from './boite-reception';

vi.useFakeTimers();

describe('BoiteReception Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: BoiteReception;
  let fixture: ComponentFixture<BoiteReception>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(BoiteReception);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 19151 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.boiteReceptions()[0]).toEqual(expect.objectContaining({ id: 19151 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 19151 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.boiteReceptions()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 19151 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to boiteReceptionService', () => {
      ...
      vi.spyOn(service, 'getBoiteReceptionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getBoiteReceptionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.boiteReceptionsParams()).toMatchObject({ sort: ['id,desc'] });
  });
